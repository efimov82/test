$(function () {
  ...
  connection.onmessage = function (message) {
    ...
    // JSON this should work without any problem but we should make sure that the
    // massage is not chunked or otherwise damaged.
    try {
    ...
    // code above first response from the server with user's color
    if (json.type === 'color') {
      myColor = json.data;
      status
        .text(myName + ': ')
  ...
  input.keydown(function (e) {
    if (e.keyCode === 13) {
      ...
      connection.send(msg);
      $(this).val('');
      // disable the input field to make the user wait until server sends back
      // response
  ...
  function addMessage(author, message, color, dt) {
    content.prepend('<p><span style="color:' + color + '">' + author + '</span> @ ' + (dt.getHours() < 10
      ? '0' + dt.getHours()
      : dt.getHours()) + ':' + (dt.getMinutes() < 10
      ? '0' + dt.getMinutes()
